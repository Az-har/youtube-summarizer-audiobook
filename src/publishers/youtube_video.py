        ...
        if uploaded_id:
            ...
            if playlist_id:
                try:
                    ...
                    print(f"  [YouTubePublisher] Added to Podcast Playlist: {playlist_id} (Available in YouTube Music!)", flush=True)
                except Exception as exc:
    ...
    def _get_or_create_playlist(self, youtube: Any) -> str:
        ...
        try:
            ...
            created = youtube.playlists().insert(
                part="snippet,status",
                body={
                    "snippet": {"title": target, "description": "Autonomous AI Audiobooks & Podcasts."},
                    "status": {"privacyStatus": self.settings.youtube_privacy_status},
                },
            ).execute()
            self._cached_playlist_id = created.get("id", "")
            print(f"  [YouTubePublisher] Auto-created Podcast Playlist: '{target}' ({self._cached_playlist_id})", flush=True)
            return self._cached_playlist_id
        except Exception as exc:
            logger.warning(f"Could not resolve podcast playlist: {exc}")
            return ""
